...
# Копирование файлов проекта (предполагается, что файлы уже скопированы в ~/guide_ai_bot_project)
# Если файлы проекта находятся в другой директории, укажите правильный путь
# Например, если файлы находятся в домашней директории:
if [ ! -f "requirements.txt" ]; then
    echo "Файл requirements.txt не найден. Копируем файлы проекта..."
    cp -r ~/guide_ai_bot_project_source/* ~/guide_ai_bot_project/
fi
...
# Убедимся, что мы в правильной директории и файл requirements.txt существует
if [ ! -f "requirements.txt" ]; then
    echo "Файл requirements.txt все еще не найден. Проверьте, что файлы проекта скопированы в ~/guide_ai_bot_project"
    exit 1
fi
